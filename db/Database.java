...
public class Database {
    ...
    private static void addFromFile(Entity e) throws InvalidEntityException {
        entities.add(e.copy());
    }
        ...
        try {
            ...
            while ((line = reader.readLine()) != null){
                if(line.contains("Task")) {
                    ...
                    try {
                        Database.addFromFile(serializer.deserialize(line));
                        if(serializer.deserialize(line).id > id){
                            id = serializer.deserialize(line).id;
                        }
                ...
                if(line.contains("User")) {
                    Serializer serializer = serializers.get(Users.USERS_ENTITY_CODE);
                    try{
                        Database.addFromFile(serializer.deserialize(line));
                        if(serializer.deserialize(line).id > id){
                            id = serializer.deserialize(line).id;
